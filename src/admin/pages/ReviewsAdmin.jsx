...
const ReviewsAdmin = () => {
  ...
  return (
    <div>
      <h1 className="mb-6 font-display text-2xl font-bold text-ink-900">Reviews</h1>
          ...
          {reviews.map((r) => (
            <div key={r._id} className="card flex items-start justify-between gap-4 p-4">
              <div>
                ...
                <p className="mt-1 text-xs text-gray-400">by {r.user?.name} · {new Date(r.createdAt).toLocaleDateString()}</p>
              </div>
              <button
